...
##            This WDL script performs alignment using Base Recalibration              ##
...
task BaseRecalibration {
   ...
   command {
      ...
      [ -s ${Aligned_Sorted_Dedupped_Realigned_Bam} ] || echo "Aligned Sorted Dedupped Bam File is Empty" >> ${Failure_Logs}
  
      # Base Recalibration detects systematic errors in base quality scores  
      ${JAVA} -Xmx16g -jar $GATK -T BaseRecalibrator -R ${RefFasta} -I ${Aligned_Sorted_Dedupped_Realigned_Bam} -knownSites ${Millsand1000GIndels} --out ${sampleName}_recal_report.grp -nct 17
