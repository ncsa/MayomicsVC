...
workflow GermlineMulti-sample-WF {

    Scatter (sampleName in samples) {
    ...
    Scatter (interval in genomic_intervals){
        if(Bqsr) {
    ...
    call GatherBamsTask {inputs:select_first(bqsr.outputs, DeliverAlignOutputBams)}
    call GatherVCFsTask {haplotype.OutputVcf, haplotype.OutputVcfIdx}
    Scatter (interval in genomic_intervals){
        call ImportGVCFs
        call GenotypeGVCFs
    }
