...
The src/ folder is broken up by language. In src/, we have 3 subfolders, (1) [Shell] (https://github.com/ncsa/MayomicsVC/blob/master/src/shell/README.md) - The folder shell consists of all the shell scripts that calls the bioinformatics software.  (2) Python - The python folder contains scripts to parse the config files for JSON and validate them for correctness. (3) WDL - The shell script is called by WDL which are the scripts of the workflow management.
